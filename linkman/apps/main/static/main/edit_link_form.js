...
const EDIT_LINK_GROUP_SELECT = document.getElementById(
    'edit-link-group-select',
);
...
function populateGroupSelect(selectedGroupId) {
    EDIT_LINK_GROUP_SELECT.innerHTML =
    ...
    utils.GROUPS.forEach((group) => {
        ...
        EDIT_LINK_GROUP_SELECT.appendChild(option);
    });
}
...
export function setUpEditLinkCloseHandlers(
    ...
    const closeHandler = () => {
        if (
            ...
            EDIT_LINK_GROUP_SELECT.value !== String(initialGroupId)
        ) {
...
export function setUpEditLinkForm(link) {
    ...
    // populate and set the group select
    populateGroupSelect(initialGroupId);
...
EDIT_LINK_FORM.addEventListener('submit', async function (e) {
    ...
    const newLinkGroupID = EDIT_LINK_GROUP_SELECT.value;
